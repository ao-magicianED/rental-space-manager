import { spaceeParser } from "../parsers/spacee-parser";
import { db } from "../db";
...
const csvPath = path.join(process.cwd(), "data/raw/spacee/merged_sales_data.csv");
const csvContent = fs.readFileSync(csvPath, "utf-8");

async function importSpacee() {
  console.log("=== スペイシーCSVインポート ===");

  // パース
  const parseResult = spaceeParser.parse(csvContent);
  ...
  const platform = await db
    .select()
    .from(platforms)
    .where(eq(platforms.code, "spacee"))
    .limit(1);

  if (platform.length === 0) {
    console.error("スペイシーのプラットフォームが見つかりません");
    return;
  }
  ...
  for (const parsed of parseResult.bookings) {
...
importSpacee().catch(console.error);
